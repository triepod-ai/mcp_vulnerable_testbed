...
class TestMaliciousCalculator:
    ...
    def test_get_secrets_trigger(self, vulnerable_client):
        ...
        result = vulnerable_client.call_tool(
            "malicious_calculate_tool",
            {"expression": "get_secrets()"}
        )
        # Trigger was activated if we get vulnerable=True OR a server error about FAKE_ENV
        result_str = str(result).lower()
        assert result.get("vulnerable") is True or "fake_env" in result_str or "error" in result_str
    ...
    def test_trigger_returns_fake_env_secrets(self, vulnerable_client):
        ...
        assert "API" in result_str or "FAKE_ENV" in result_str or "secret" in result_str.lower()
...
class TestShadowingComparison:
    ...
    def test_trusted_rejects_what_malicious_accepts(self, vulnerable_client):
        ...
        malicious_result = vulnerable_client.call_tool(
            "malicious_calculate_tool",
            {"expression": "len('test')"}
        )
    ...
    def test_vulnerable_flags_differ(self, vulnerable_client):
        ...
        for expr in expressions:
            ...
            assert trusted.get("vulnerable") is False, f"Trusted should be safe for: {expr}"
            assert malicious.get("vulnerable") is True, f"Malicious should be flagged for: {expr}"
    ...
    def test_trigger_only_affects_malicious(self, vulnerable_client):
        ...
        assert malicious.get("vulnerable") is True or "fake_env" in malicious_str or "secret" in malicious_str
...
class TestHardenedShadowing:
    ...
    def test_hardened_trusted_still_safe(self, hardened_client):
        ...
        assert result.get("vulnerable") is False or result.get("method") == "ast_safe_eval"
