...
class TestRugPullBoundary:
    ...
    def test_rug_pull_exact_boundary_invocation_10(self, clean_vulnerable_client):
        ...
        for i in range(1, 11):
            result = clean_vulnerable_client.call_tool(
                "vulnerable_rug_pull_tool", {"action": f"safe_action_{i}"}
            )
    ...
    def test_rug_pull_exact_boundary_invocation_11(self, clean_vulnerable_client):
        ...
        for i in range(1, 11):
            clean_vulnerable_client.call_tool(
                "vulnerable_rug_pull_tool", {"action": f"buildup_{i}"}
            )

        # 11th call should trigger
        result = clean_vulnerable_client.call_tool(
            "vulnerable_rug_pull_tool", {"action": "trigger_action"}
        )
        ...
        assert any(
            indicator in result_str for indicator in ["executed", "root", "command"]
    ...
    def test_rug_pull_stays_malicious_after_trigger(self, clean_vulnerable_client):
        ...
        for i in range(12, 21):
            result = clean_vulnerable_client.call_tool(
                "vulnerable_rug_pull_tool", {"action": f"post_trigger_{i}"}
            )
    ...
    def test_rug_pull_boundary_off_by_one_prevention(self, clean_vulnerable_client):
        ...
        for i in range(1, 15):
            result = clean_vulnerable_client.call_tool(
                "vulnerable_rug_pull_tool", {"action": f"test_{i}"}
            )
            results.append((i, result.get("vulnerable", False)))

        # Find first trigger point
        trigger_point = next((i for i, is_vuln in results if is_vuln), None)
...
class TestRugPullStatePersistence:
    ...
    def test_state_persists_across_different_payloads(self, clean_vulnerable_client):
        ...
        for i, payload in enumerate(diverse_payloads, 1):
            result = clean_vulnerable_client.call_tool(
                "vulnerable_rug_pull_tool", {"action": payload}
            )
        ...
        result = clean_vulnerable_client.call_tool(
            "vulnerable_rug_pull_tool", {"action": "innocent_looking_action"}
        )
    ...
    def test_state_persists_with_interleaved_tool_calls(self, clean_vulnerable_client):
        ...
        for i in range(5):
            clean_vulnerable_client.call_tool(
                "vulnerable_rug_pull_tool", {"action": f"call_{i}"}
            )
        ...
        clean_vulnerable_client.call_tool("safe_storage_tool_mcp", {"data": "test"})
        ...
        result = clean_vulnerable_client.call_tool(
            "vulnerable_rug_pull_tool", {"action": "final_call"}
        )
        assert result.get("vulnerable") is True, "11th rug pull call should trigger"
...
class TestResetBehavior:
    ...
    def test_reset_clears_rug_pull_state(self, vulnerable_client):
        ...
        for i in range(15):
            vulnerable_client.call_tool(
                "vulnerable_rug_pull_tool", {"action": f"pre_reset_{i}"}
            )

        # Verify we're in malicious state
        result = vulnerable_client.call_tool(
            "vulnerable_rug_pull_tool", {"action": "verify_malicious"}
        )
        ...
        result = vulnerable_client.call_tool(
            "vulnerable_rug_pull_tool", {"action": "verify_safe"}
        )
    ...
    def test_multiple_reset_cycles(self, vulnerable_client):
        ...
        for cycle in range(3):
            ...
            # Verify starts safe
            result = vulnerable_client.call_tool(
                "vulnerable_rug_pull_tool", {"action": f"cycle_{cycle}_start"}
            )
            assert result.get("vulnerable") is False, (
            ...
            for i in range(10):
                vulnerable_client.call_tool(
                    "vulnerable_rug_pull_tool", {"action": f"cycle_{cycle}_buildup_{i}"}
                )

            # Verify triggered
            result = vulnerable_client.call_tool(
                "vulnerable_rug_pull_tool", {"action": f"cycle_{cycle}_trigger"}
            )
            assert result.get("vulnerable") is True, (
...
class TestHardenedConsistency:
    ...
    def test_hardened_never_triggers_rug_pull(self, hardened_client):
        ...
        for i in range(1, 31):
            result = hardened_client.call_tool(
                "vulnerable_rug_pull_tool", {"action": f"test_call_{i}"}
            )
    ...
    def test_hardened_consistent_response_pattern(self, hardened_client):
        ...
        for i in range(20):
            result = hardened_client.call_tool(
                "vulnerable_rug_pull_tool", {"action": "consistent_test"}
            )
            responses.append(result)

        # All responses should have similar structure
        set(responses[0].keys())
...
class TestEdgeCases:
    ...
    def test_rapid_fire_calls(self, clean_vulnerable_client):
        ...
        for i in range(1, 16):
            result = clean_vulnerable_client.call_tool(
                "vulnerable_rug_pull_tool", {"action": "rapid"}
            )
            results.append(
                (i, result.get("vulnerable", False), result.get("invocation_count"))
            )
