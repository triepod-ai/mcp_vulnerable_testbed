...
class TestChallenge8IndirectInjection:
    ...
    def test_important_tag_injection(self, vulnerable_client):
        ...
        result = vulnerable_client.call_tool(
            "vulnerable_document_processor_tool", {"content": payload}
        )
        ...
        assert result.get("injection_patterns_detected") is True, (
    ...
    def test_evidence_field_present(self, vulnerable_client):
        ...
        assert (
            "indirect" in evidence.lower()
            or "unsanitized" in evidence.lower()
            or "raw" in evidence.lower()
...
class TestChallenge9SecretLeakage:
    ...
    def test_verbose_database_credentials(self, vulnerable_client):
        ...
        assert any(
            ...
            for indicator in ["postgres://", "DATABASE_URL", "connection_string"]
    ...
    def test_verbose_api_gateway_credentials(self, vulnerable_client):
        ...
        assert any(
            indicator in result_str
            for indicator in ["sk-test", "API_KEY", "api_key", "SECRET_TOKEN", "Bearer"]
    ...
    def test_error_path_credential_leak(self, vulnerable_client):
        ...
        has_leak = any(
            indicator in result_str
            for indicator in ["debug_context", "internal_error", "sk-test", "API_KEY"]
        )
    ...
    def test_aws_credentials_exposure(self, vulnerable_client):
        ...
        result = vulnerable_client.call_tool(
            "vulnerable_service_status_tool", {"service_name": "aws", "verbose": True}
        )
        ...
        assert any(
            indicator in result_str
            for indicator in ["AKIA", "AWS_ACCESS_KEY", "access_key", "SecretKey"]
        ), "AWS credentials should be exposed in verbose mode"
...
class TestChallenge10NetworkInjection:
    ...
    def test_semicolon_injection(self, vulnerable_client):
        ...
        assert result.get("injection_detected") is True, (
            "Injection pattern should be detected"
        )
...
class TestChallenge11BlacklistBypass:
    ...
    def test_blocked_rm_correctly(self, vulnerable_client):
        ...
        result = vulnerable_client.call_tool(
            "vulnerable_safe_executor_tool", {"command": "rm -rf /"}
        )
    ...
    def test_perl_bypass(self, vulnerable_client):
        ...
        result = vulnerable_client.call_tool(
            "vulnerable_safe_executor_tool", {"command": "perl -e 'print \"bypassed\"'"}
        )
    ...
    def test_wget_bypass(self, vulnerable_client):
        ...
        assert (
            result.get("bypass_used") is True
            or result.get("blacklist_check") == "passed"
...
class TestHardenedVersionsNotFlagged:
    ...
    def test_hardened_service_status_not_flagged(self, hardened_client):
        ...
        assert "postgres://" not in result_str, (
            "No connection strings should be in response"
        )
    ...
    def test_hardened_network_diagnostic_not_flagged(self, hardened_client):
        ...
        no_execution = (
            result.get("command_executed") is False
            or result.get("status") == "rejected"
            or "no command executed" in str(result.get("note", "")).lower()
        )
        assert no_execution, "Hardened version should NOT execute commands"
    ...
    def test_hardened_safe_executor_not_flagged(self, hardened_client):
        ...
        result = hardened_client.call_tool(
            "vulnerable_safe_executor_tool", {"command": "python3 -c 'print(1)'"}
        )
    ...
    def test_hardened_session_not_flagged(self, hardened_client):
        ...
        has_secure_indicator = (
            result.get("token_secure") is True
            or "secrets" in note
            or "cryptographically secure" in note
            or "secure token" in note
        )
    ...
    def test_hardened_crypto_not_flagged(self, hardened_client):
        ...
        assert result.get("algorithm") != "MD5", "Hardened version should NOT use MD5"
    ...
    def test_hardened_encryption_not_flagged(self, hardened_client):
        ...
        assert result.get("mode") != "ECB", "Hardened version should NOT use ECB mode"
...
class TestChallenge12SessionVulnerabilities:
    ...
    def test_no_regeneration_detected(self, clean_vulnerable_client):
        ...
        assert login_result.get("vulnerable") is True, (
            "Tool should be marked vulnerable"
        )
...
class TestChallenge13CryptoVulnerabilities:
    ...
    def test_md5_hashing_detected(self, vulnerable_client):
        ...
        assert result.get("vulnerable") is True, "Tool should be marked vulnerable"
        assert result.get("algorithm") == "MD5", "Algorithm should be MD5 (broken)"
        assert result.get("algorithm_secure") is False, (
            "algorithm_secure should be False for MD5"
        )
        assert "CWE-328" in result.get("cwe_ids", []), "CWE-328 should be reported"
    ...
    def test_static_salt_detected(self, vulnerable_client):
        ...
        assert result.get("salt_type") == "static", "salt_type should be 'static'"
        assert result.get("salt_secure") is False, "salt_secure should be False"
        assert "CWE-916" in result.get("cwe_ids", []), "CWE-916 should be reported"

    @pytest.mark.integration
    def test_predictable_rng_detected(self, vulnerable_client):
        """CWE-330: Predictable RNG should be detected."""
        result = vulnerable_client.call_tool(
            "vulnerable_crypto_tool_endpoint", {"password": "dummy", "action": "random"}
        )
        ...
        assert "CWE-330" in result.get("cwe_ids", []), "CWE-330 should be reported"
    ...
    def test_timing_attack_detected(self, vulnerable_client):
        ...
        assert result.get("vulnerable") is True, "Tool should be marked vulnerable"
        assert result.get("timing_safe") is False, "timing_safe should be False"
        assert result.get("comparison_type") == "direct_equality", (
            "comparison_type should indicate non-constant-time"
        )
        assert "CWE-208" in result.get("cwe_ids", []), "CWE-208 should be reported"
...
class TestChallenge13EncryptionVulnerabilities:
    ...
    def test_ecb_mode_detected(self, vulnerable_client):
        ...
        assert "CWE-327" in result.get("cwe_ids", []) or "CWE-321" in result.get(
            "cwe_ids", []
    ...
    def test_weak_kdf_detected(self, vulnerable_client):
        ...
        assert result.get("iterations") == 1, "iterations should be 1 (no stretching)"
        assert result.get("salt_used") is False, "salt_used should be False"
        assert result.get("kdf_secure") is False, "kdf_secure should be False"
        assert "CWE-916" in result.get("cwe_ids", []), "CWE-916 should be reported"
    ...
    def test_weak_hmac_key_detected(self, vulnerable_client):
        ...
        assert result.get("vulnerable") is True, "Tool should be marked vulnerable"
        assert result.get("key_length") == 3, "key_length should be 3 (weak)"
        assert result.get("key_secure") is False, "key_secure should be False"
        assert "CWE-326" in result.get("cwe_ids", []), "CWE-326 should be reported"
