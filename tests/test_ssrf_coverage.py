...
class TestCloudMetadataEndpoints:
    ...
    @pytest.mark.parametrize("url,provider,description", [
        # AWS
        pytest.param(
    ...
    def test_cloud_metadata_ssrf(
        ...
        assert "ssrf" in result_str or "internal" in result_str or "169.254" in result_str, \
...
class TestInternalNetworkRanges:
    ...
    @pytest.mark.parametrize("url,network_class", [
        # Class A private
        pytest.param("http://10.0.0.1/admin", "10.x.x.x", id="class-a-10-0-0-1"),
        pytest.param("http://10.255.255.255/", "10.x.x.x", id="class-a-10-255"),
        # Class B private
        pytest.param("http://172.16.0.1/config", "172.16-31.x.x", id="class-b-172-16"),
        pytest.param("http://172.31.255.255/", "172.16-31.x.x", id="class-b-172-31"),
        # Class C private
        pytest.param("http://192.168.0.1/", "192.168.x.x", id="class-c-192-168-0"),
        pytest.param("http://192.168.255.255/", "192.168.x.x", id="class-c-192-168-255"),
        # Link-local
...
class TestLocalhostVariants:
    ...
    @pytest.mark.parametrize("url,variant", [
        # Standard localhost
        pytest.param("http://localhost/admin", "localhost", id="localhost-word"),
        pytest.param("http://localhost:8080/", "localhost:port", id="localhost-port"),
        # IPv4 localhost
        ...
        pytest.param("http://0.0.0.0:9200/", "0.0.0.0:port", id="ipv4-elasticsearch"),
...
class TestIPv6SSRF:
    ...
    @pytest.mark.parametrize("url,description", [
        ...
        pytest.param("http://[::1]:8080/", "IPv6 localhost with port", id="ipv6-localhost-port"),
...
class TestURLBypassTechniques:
    ...
    @pytest.mark.parametrize("url,technique", [
        # IP encoding tricks
        pytest.param(
            "http://2130706433/",
    ...
    def test_ssrf_bypass_techniques(
        self,
        vulnerable_client,
        url: str,
        technique: str
...
class TestFileProtocolSSRF:
    ...
    @pytest.mark.parametrize("url,target", [
        ...
        pytest.param("file:///proc/self/environ", "process environment", id="proc-environ"),
        pytest.param("file:///proc/self/cmdline", "process cmdline", id="proc-cmdline"),
        pytest.param("file:///app/config.py", "application config", id="app-config"),
    ])
    def test_file_protocol_ssrf(
        self,
        vulnerable_client,
        url: str,
        target: str
