...
class TestSafeToolBoundaries:
    ...
    def test_safe_storage_input_boundary(
        self, hardened_client, size: int, should_accept: bool
    ):
        """Test safe_storage_tool_mcp enforces 10KB boundary exactly."""
        payload = "A" * size

        result = hardened_client.call_tool("safe_storage_tool_mcp", {"data": payload})
        ...
        if should_accept:
            ...
            assert (
                "error" in result_str
                or "exceeds" in result_str
                or "too long" in result_str
            ), f"Size {size}: Should reject oversized input. Got: {result_str[:200]}"

    @pytest.mark.parametrize(
        "tool_name,arg_name,max_expected",
        [
            ...
            (
                ...
                "input_data",
                1000,
            ),  # Validate has 1KB business limit
        ],
    )
    def test_safe_tools_have_limits(
        self, hardened_client, tool_name: str, arg_name: str, max_expected: int
    ):
        ...
        assert (
            "error" in reject_str or "exceeds" in reject_str or "too long" in reject_str
        ), (
...
class TestVulnerableToolsNoLimits:
    ...
    def test_vulnerable_calculator_no_size_limit(self, vulnerable_client, size: int):
    ...
    def test_vulnerable_tools_accept_oversized_input(
        self, vulnerable_client, tool_name: str, arg_name: str
    ):
...
class TestDifferentialDoSBehavior:
    ...
    def test_differential_oversized_input(self, vulnerable_client, hardened_client):
        ...
        vulnerable_client.call_tool(
            "safe_storage_tool_mcp", {"data": oversized_payload}
        )
...
class TestUnicodeEncodingBypass:
    ...
    def test_unicode_encoded_input_size(self, hardened_client):
        ...
        result = hardened_client.call_tool(
            "safe_storage_tool_mcp", {"data": unicode_payload}
        )
    ...
    def test_multibyte_unicode_characters(self, hardened_client):
        ...
        result = hardened_client.call_tool(
            "safe_storage_tool_mcp", {"data": emoji_payload}
        )
...
class TestBoundaryEdgeCases:
    ...
    def test_empty_input_accepted(self, hardened_client):
        ...
        result = hardened_client.call_tool("safe_storage_tool_mcp", {"data": ""})
    ...
    def test_whitespace_only_input(self, hardened_client):
        ...
        result = hardened_client.call_tool(
            "safe_storage_tool_mcp", {"data": whitespace_payload}
        )
        ...
        result_over = hardened_client.call_tool(
            "safe_storage_tool_mcp", {"data": over_whitespace}
        )
...
class TestDoSVulnerabilityProof:
    ...
    def test_prove_dos_vulnerability_exists(self, vulnerable_client):
        ...
        print(
            "Vulnerable tools accept inputs of 10KB, 50KB, 100KB without size validation"
        )
